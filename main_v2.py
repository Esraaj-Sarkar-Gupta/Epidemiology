...
class particle():
    ...
    def update_infec(self, infection_update, body): # Update properties of the element based on collision status     
        if infection_update == 0: # No collision
            self.viral_decay()
        elif infection_update == 1: # Collision
            self.infection(body)
    ...
    def infection(self, body):
        ...
        log(f"Infection of {self} and {body} gives probability P = {probability}")
        ...
        if probability >= transmission_threshold:
            ...
            log(f"Viral load of {self} increased by {delta}")
            
    def housekeeper(self):
        # Update status based on viral_load
        if self.viral_load >= virulence:
            self.status = "severely_infected"
        elif self.viral_load >= 0.50 * virulence:
            self.status = "infected"
        elif self.viral_load >= 0.25 * virulence:
            self.status = "carrier"
        else:
            self.status = "safe"
    ...
    def color(self):
        if self.status == "severely_infected":
            return "black"
        elif self.status == "infected":
            return "red"
        elif self.status == "carrier":
            return "orange"
...
def count(element_list):
    safe, carrier, infected, severely_infected, uncat = 0, 0, 0, 0, 0
    for element in element_list:
        if element.status == 'safe': safe += 1;
        elif element.status == 'carrier': carrier += 1;
        elif element.status == 'infected': infected += 1;
        elif element.status == 'severely_infected': severely_infected += 1;
        else: uncat += 1
    #print(f"Safe: {safe}\nCarrier: {carrier}\nInfected: {infected}\nSeverely Infected: {severely_infected}\nUncategorised: {uncat}")
    return safe, carrier, infected, severely_infected, uncat
...
def update(t):
    for t in range(t):
        ...
        number_safe, number_carrier, number_infected, number_severely_infected, number_uncat = count(elements)
       
        safe_count.append(number_safe)
        carrier_count.append(number_carrier)
        infected_count.append(number_infected)
        s_infected_count.append(number_severely_infected)
        uncat_count.append(number_uncat)
...
plt.plot(range(number_of_steps), safe_count, color = 'blue')
plt.plot(range(number_of_steps), carrier_count, color = 'orange')
plt.plot(range(number_of_steps), infected_count, color = 'red')
plt.plot(range(number_of_steps), s_infected_count, color = 'black')
plt.plot(range(number_of_steps), uncat_count, color = 'gray')
