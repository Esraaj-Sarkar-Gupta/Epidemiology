...
project_dir = f"./Runtime_{T[7]}_{T[3]}-{T[4]}-{T[5]}"

os.makedirs(project_dir)
os.makedirs(project_dir + '/Logs')
os.makedirs(project_dir + '/Figures')
...
def run(decay, infec):
    ...
    infectiousness = infec
    log_filename = project_dir + '/Logs/ifc=' + str(infec) + "_d=" + str(decay) + '.txt'
    save_dir = project_dir + '/Figures/ifc=' + str(infec) + "_d=" + str(decay)
    ...
    plt.figure(figsize=(10,10))
    plt.title(f"Evolution of System - Infectiousness = {infec}, Decay = {decay}")
    ...
    plt.grid(True)
    plt.savefig(f"{project_dir}/infec={infec}_decay={decay}.png")
...
for decay_value in range(0, 10 + 1):
    for infec_value in [0.5, 1, 1.5, 2, 2.5]:
        print("> Running Infectiousness = {infec_value} and Decay Constant= {decay_value / 1000}")
        run(decay_value / 1000, infec_value)
